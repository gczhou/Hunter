...
impl fmt::Display for Style {
    fn fmt(&self, f: &mut fmt::Formatter) -> Result<(), fmt::Error> {
        match self.fg {
            Some(ref fg) => {
                f.write_str(fg.get_fg_str());
            },
            None => ()
        }
        match self.bg {
            Some(ref bg) => {
                f.write_str(bg.get_bg_str());
            },
            None => ()
        }
...
fn get_prompt_input(prompt_text: &str, hide_input: bool) -> String {
    ...
    let input = io::stdin().lock().read_to_string(&mut buffer).expect("Fail to read the input");
    return buffer.trim_right_matches("\n").to_string();
}
